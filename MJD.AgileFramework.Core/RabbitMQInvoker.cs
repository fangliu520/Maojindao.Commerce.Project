...
using MJD.AgileFramework.Common.IOCOptions;

namespace MJD.AgileFramework.Core
{
    /// <summary>
    ...
    public class RabbitMQInvoker
    {
        ...
        public void SendDelay(string targetExchangeName, string message, int delaySecond)
        {
            ...
            using (var channel = _CurrentConnection.CreateModel())//开辟新的信道通信
            {
                try
                {
                    string delayExchangeName = "MJDMSA_DelayExchange";
                    ...
                    channel.QueueDeclare("MJDMSA_DelayQueue", true, false, false, args);
                    channel.QueueBind(queue: "MJDMSA_DelayQueue",
                        exchange: delayExchangeName,
                        routingKey: string.Empty,
                        arguments: null);

                    ////DLX--- //死信队列绑定
                    //channel.ExchangeDeclare("MJDMSA_exchange_dlx", "fanout", true, false, null);
                    //channel.QueueDeclare("MJDMSA_queue_dlx", true, false, false, null);
                    //channel.QueueBind("MJDMSA_queue_dlx", "MJDMSA_exchange_dlx", "routingkey", null);
